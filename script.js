...
function addOrReplaceLocalTrack(track, stream) {
  if (!pc) return;
  const kind = track.kind;
  // Try to find an existing sender for this kind
  const sender = pc.getSenders().find(s => s.track && s.track.kind === kind) ||
                 pc.getSenders().find(s => !s.track && s.transport && s.transport.state !== 'closed' && s?.track?.kind === kind);
  if (sender && sender.replaceTrack) {
    try { sender.replaceTrack(track); return; } catch (e) { console.warn('replaceTrack failed:', e); }
  }
...
// ===== WebRTC state =====
let callChannel = null; // Supabase Realtime channel for signaling/presence per room
let pc = null; // RTCPeerConnection
let negotiating = false; // negotiation lock
let localStream = null;
let remoteStream = null;
let isCaller = false;
let micEnabled = true;
let camEnabled = true;
let peerPresent = false; // whether other user is online in this DM room
...
async function switchRoom(roomId, title) {
  ...
  // Setup/teardown call signaling per room
...
(async function init() {
  ...
  loadMyProfile();
  // Refresh self from DB to get the latest avatar/bio
  try {
  ...
  // Call UI events
  callBackdrop?.addEventListener('click', endCall);
  ...
  try {
    ...
    if (isMobile()) {
      ...
      // Ensure call controls hidden on mobile initial screen (no room yet)
      updateCallButtonsState();
    } else {
...
// ===== Calls: helpers and signaling =====
function updateCallButtonsState() {
  const inDM = currentRoomId.startsWith('dm:');
  // Do not disable buttons; only hide controls outside of DMs.
  // startCall() will guard invalid cases.
  if (callVoiceBtn) callVoiceBtn.disabled = false;
  if (callVideoBtn) callVideoBtn.disabled = false;
  // Hide controls completely outside of DMs
  if (callControls) callControls.classList.toggle('hidden', !inDM);
}

function setupCallChannel(roomId) {
  console.log('🔧 Setting up call channel for room:', roomId);
  // Leave previous channel
  if (callChannel) try { supabase.removeChannel(callChannel); } catch {}
  peerPresent = false;
  if (!roomId.startsWith('dm:')) { 
    console.log('❌ Not a DM room, skipping call setup');
    updateCallButtonsState(); 
    return; 
  }
  
  const channelName = `call:${roomId}`;
  console.log('📡 Creating call channel:', channelName);
  
  // Use a dedicated channel for signaling with presence
  callChannel = supabase.channel(channelName, {
    config: { presence: { key: me?.id || Math.random().toString(36).slice(2) } }
  });
  ...
  callChannel
    .on('presence', { event: 'sync' }, () => {
      try {
        ...
        // state is a map of userId -> metas[]; consider present if >= 2 keys
        const ids = Object.keys(state || {});
        console.log('👤 Present user IDs:', ids);
        console.log('🆔 My ID:', me.id);
        peerPresent = ids.some(id => id !== me.id);
        console.log('🟢 Peer present:', peerPresent);
        updateCallButtonsState();
    ...
    .on('broadcast', { event: 'webrtc' }, async ({ payload }) => {
      ...
      try { await handleSignal(payload); } catch (e) { console.warn('signal err:', e); }
    })
    .subscribe(async (status) => {
      console.log('📡 Channel subscription status:', status);
      if (status === 'SUBSCRIBED') {
        try { 
          await callChannel.track({ nick: me?.nick || 'user' }); 
          console.log('✅ Tracked presence for user:', me?.nick);
        } catch (e) { console.error('Track presence error:', e); }
      }
    });
}

async function broadcastSignal(data) {
  console.log('📤 Broadcasting signal:', data);
  try {
    await callChannel?.send({ type: 'broadcast', event: 'webrtc', payload: { ...data, from: me.id } });
...
async function ensureMedia(video) {
  // If we already have a stream, but need to upgrade to video, try to add a video track
  if (localStream) {
  ...
  try {
    ...
    if (localVideo) {
      try { localVideo.muted = true; } catch {}
      localVideo.srcObject = localStream;
      try { await localVideo.play(); } catch (e) { console.warn('localVideo.play() failed:', e); }
    }
    micEnabled = true; camEnabled = !!video;
    return localStream;
  } catch (error) {
    console.warn('Media access error:', error);
    if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
      throw new Error('Доступ к камере/микрофону отклонён. Разрешите доступ в настройках браузера.');
    } else if (error.name === 'NotFoundError') {
      throw new Error('Камера или микрофон не найдены.');
    } else if (error.name === 'NotReadableError') {
      throw new Error('Устройство занято другой программой. Закройте другие приложения, использующие камеру/микрофон.');
    } else {
...
function attachRemoteStream() {
  ...
  if (remoteVideo) {
    ...
    // Try to autoplay remote video when tracks arrive
    setTimeout(async () => {
      try { await remoteVideo.play(); } catch (e) { console.warn('remoteVideo.play() failed:', e); }
    }, 0);
  }
}
...
function createPeer() {
  ...
  pc.onicecandidate = (e) => {
    if (e.candidate) broadcastSignal({ type: 'ice', candidate: e.candidate });
  };
  pc.ontrack = (e) => {
    console.log('🎞️ ontrack:', e.track?.kind, e);
    attachRemoteStream();
    if (e.streams && e.streams[0]) {
      // Some browsers provide full MediaStream
      e.streams[0].getTracks().forEach(t => remoteStream.addTrack(t));
    } else if (e.track) {
      // Fallback: add single track
      try { remoteStream.addTrack(e.track); } catch (err) { console.warn('addTrack fallback failed:', err); }
    }
  };
  ...
  // Pre-create transceivers to negotiate both directions even if track not ready yet
  try {
  ...
  pc.onnegotiationneeded = async () => {
    if (!pc) return;
    if (negotiating) { console.log('⏳ Skip negotiation: already in progress'); return; }
    negotiating = true;
    try {
      console.log('🤝 onnegotiationneeded -> createOffer');
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      await broadcastSignal({ type: 'offer', sdp: offer });
    } catch (e) {
...
async function startCall(withVideo) {
  console.log('📞 Starting call, video:', withVideo);
  console.log('🏠 Current room:', currentRoomId);
  console.log('👥 Peer present:', peerPresent);
  
  if (!currentRoomId.startsWith('dm:')) {
    console.log('❌ Not in DM room');
    return;
  }
  
  // Remove peer presence check for debugging
  console.log('🚀 Proceeding with call regardless of peer presence for debugging');
  ...
  try {
    ...
    stream.getTracks().forEach(track => addOrReplaceLocalTrack(track, stream));
...
async function handleSignal(msg) {
  if (!msg || msg.from === me.id) return; // ignore self
  switch (msg.type) {
    case 'offer': {
      // Auto-accept for now
      isCaller = false;
      try {
        callModal?.classList.remove('hidden');
        createPeer();
        const wantVideo = !!(msg.sdp?.sdp && /m=video/.test(msg.sdp.sdp));
        const stream = await ensureMedia(wantVideo);
        stream.getTracks().forEach(track => addOrReplaceLocalTrack(track, stream));
        await pc.setRemoteDescription(new RTCSessionDescription(msg.sdp));
        const answer = await pc.createAnswer();
        await pc.setLocalDescription(answer);
        await broadcastSignal({ type: 'answer', sdp: answer });
      } catch (e) {
        console.warn('handleOffer:', e);
...
function endCall(broadcast = true) {
  try { callModal?.classList.add('hidden'); } catch {}
  ...
  if (broadcast) broadcastSignal({ type: 'end' });
  updateCallButtonsState();
}
...
function toggleMic() {
  ...
  toggleMicBtn.textContent = micEnabled ? 'Микрофон' : 'Микрофон выкл';
}

function toggleCam() {
  (async () => {
    if (!localStream) {
      // If stream not started (например, аудиовызов) — попробуем включить камеру
      try {
        await ensureMedia(true);
      } catch (e) {
        alert(e.message || 'Не удалось включить камеру');
        return;
      }
    }
    // If no video tracks yet, try to add one
    ...
    if (toggleCamBtn) toggleCamBtn.textContent = camEnabled ? 'Камера' : 'Камера выкл';
  })();
}
