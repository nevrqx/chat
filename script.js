...
function getAvatarUrlFor(userId, nick) {
  if (me && userId === me.id) return myProfile.avatar_url || placeholderAvatar(me.nick, 64);
  // В будущем можно получать из БД. Пока — плейсхолдер по нику/ID
...
function openProfile(user) {
  ...
  profileBio.textContent = isSelf ? (myProfile.bio || 'Без описания') : 'Нет описания';
...
(async function init() {
  ...
  // Profile load and UI init
  loadMyProfile();
  const myAvatarUrl = myProfile.avatar_url || placeholderAvatar(me.nick, 64);
  ...
  if (meBioTextarea) meBioTextarea.value = myProfile.bio || '';
