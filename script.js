...
function updateCallButtonsState() {
  ...
  // Enable in DM even if peer not present, so buttons are clickable; we'll block on startCall
  const enabled = inDM && !pc;
  if (callVoiceBtn) callVoiceBtn.disabled = !enabled;
  if (callVideoBtn) callVideoBtn.disabled = !enabled;
...
function setupCallChannel(roomId) {
  ...
  peerPresent = false;
  if (!roomId.startsWith('dm:')) { updateCallButtonsState(); return; }
  // Use a dedicated channel for signaling with presence
  callChannel = supabase.channel(`call:${roomId}`, {
  ...
  callChannel
    ...
    .subscribe(async (status) => {
      if (status === 'SUBSCRIBED') {
        try { await callChannel.track({ nick: me?.nick || 'user' }); } catch {}
      }
    });
}
...
async function startCall(withVideo) {
  if (!currentRoomId.startsWith('dm:')) return;
  if (!peerPresent) { alert('Собеседник не в сети в этом ЛС. Откройте чат на обоих устройствах.'); return; }
  isCaller = true;
  try {
...
function toggleCam() {
  ...
  toggleCamBtn.textContent = camEnabled ? 'Камера' : 'Камера выкл';
}
