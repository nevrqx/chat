...
function upsertRecentDM({ roomId, peerId, peerNick, lastAt, lastText, incUnread = 0 }) {
  ...
  if (idx === -1) {
    recentDMs.unshift({ roomId, peerId, peerNick: peerNick || peerId.slice(0,8)+'…', lastAt: lastAt || new Date().toISOString(), lastText: lastText || '', unread: incUnread ? 1 : 0 });
  } else {
...
async function openDM(user) {
  ...
  upsertRecentDM({ roomId, peerId: user.id, peerNick: user.nick, lastAt: new Date().toISOString(), lastText: '', incUnread: 0 });
